...
if exist "%~dp0ERP_Code\nuget.exe" (
    "%~dp0ERP_Code\nuget.exe" restore "%~dp0ERP_Code\ERP.sln" >nul 2>&1
) else if exist "%~dp0nuget.exe" (
    "%~dp0nuget.exe" restore "%~dp0ERP_Code\ERP.sln" >nul 2>&1
)

echo [2/3] Dang bien dich toan bo giai phap ERP.sln...
dotnet msbuild "%~dp0ERP_Code\ERP.sln" /t:Build /p:Configuration=Debug
...
start "" "%~dp0ERP_Code\ERP_BanHang\ERP_Khach\bin\Debug\ERP_Khach.exe"
